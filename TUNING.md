...
| Depth fog | `FogFeel` (`GameFeel.fog`) | The outside-water darkness gradient that CLOSES IN on the sub with depth: `fog_color` (deep navy), `zone_caps` (depth_m -> darkness-alpha breakpoints, used by `darkness_alpha()`/`outer_alpha()` to drive the darken curve — Shallows fog-free, then a per-zone ceiling; rescaled by Snir to match `cavern_depths_01`'s real painted depth). `outer_alpha()` re-normalizes `darkness_alpha()`'s own multi-stage zone curve onto a 0..1 range, so the far field darkens GRADUALLY through each zone threshold rather than snapping straight to black the moment any fog starts — only reaching fully opaque at the deepest zone cap. `clear_radius_anchors` (2026-06-29 follow-up #4, Snir) is a SEPARATE list of (depth_m, radius_m) anchor points, on the same depths as `zone_caps`: the visible "hug" area starts as effectively the whole screen (500m) at the first threshold, then shrinks at each subsequent anchor (35m, 15m) down to a small hull-hugging contour (7m) at the deepest zone — `clear_radius_m_at()` linearly interpolates between anchors the same way `darkness_alpha()` interpolates `zone_caps`. Plus `falloff_width_m` (6m, how wide the gradient ring is). This clear area is measured from the sub's actual hull rects (`shaders/depth_fog.gdshader`'s `hull_rects[16]` uniform, fed from `Sub.hull_rects()` each frame), not a single center point, so it hugs the hull's real (lumpy) silhouette instead of being a circle (follow-up #3). The floodlight genuinely "repels" the dark — `floodlight_cutout_softness_m` (3m) is the soft edge on the beam's repelled-darkness cone, carved directly into the same shader from the active `FloodlightStation`'s aim/reach (`Sub.active_floodlight()`), not just painted on top. `ambient_bubble_radius_m` (3m, past the hull) is just the cosmetic warm-highlight overlay drawn in `sub_visual.gd`, separate from the real visibility radius. Cosmetic only — never gates vision/AI/detection. Beam reach/cone/falloff itself stays in `FloodlightFeel` (`max_alpha` is now 0.04, near-transparent as of follow-up #3 — Snir: the real indication of the floodlight should be the darkness it repels, not the painted beam color; was briefly 0.85, originally 0.35). |
