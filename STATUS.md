...
_Read this at session start. Last updated: 2026-06-29 — **M11 shipped THE DEEP DARK** (depth fog + dock-return fix), then FOUR same-day/next-day follow-ups, each refining the fog feel: (1) radial gradient fog + Floodlight Room added to the base sub, (2) far field made fully opaque + camera zoom halved + floodlight carving a real wedge into the darkness, (3) that far-field darkening made GRADUAL instead of a snap, the clear area reshaped to hug the sub's actual hull silhouette instead of a circle, the floodlight's painted color made near-transparent, and the foreground map layer moved to sit beneath the fog, (4) the visible "hug" radius now starts as the WHOLE SCREEN at the first depth threshold and shrinks at predefined anchor points toward the small hull-hugging contour by the deepest zone, instead of always being small. Next: **the mid-build fog tuning pass** (`clear_radius_anchors`/`falloff_width_m`/`floodlight_cutout_softness_m` are all first-pass numbers from this session, want Snir's feel pass after playing with the latest version), **THE DISCHARGER**, the carried-over **economy balance pass** (re-parked again out of M11), and a design call flagged below — **the wider hull (now 5 cells) no longer fits through a cave passage in `cavern_depths_01`** that the 4-room sub used to clear._
...
- **Floodlight Room added to the base sub:** `SubLayout.starting_layout()`
  ...
  **leftmost** slot (was 4 rooms, now 5: floodlight_room, telescope_room,
  helm, bullet_room, tower), with its own `floodlight_pod` pre-attached.
  `telescope_room`'s reach face moved from **left → bottom** (the floodlight
  ...
  - **Side effect — the hull got one cell (5m) physically wider,** and the
    geometry builder re-centers the whole bounding box around the sub's
    origin, so **both** the bow and stern edges sit ~2.5m further out than
    before (not just the side the new room was added to). Tests that placed
    things at fixed offsets from the sub (fish territory ranges, a chaser's
    line-of-sight wall, the ranged-attack precondition distance) now measure
    against the sub's real hull edge (`sub.hull_rects()`) instead of a
    hardcoded "~8.3m-wide hull" constant.
  - **Open design flag for Snir:** `tests/test_world.gd`'s cave test now
    fails — the wider hull can no longer fit through a passage in
    `cavern_depths_01` that the old 4-room sub cleared. Left the test
    assertion as-is (it's correctly catching a real gameplay change, not a
...
**Fish and dropped currency stranded in open air now fall under plain gravity, no steering (2026-06-24):** follow-up to the sky-freeze fix below — Snir flagged that once unstuck, a stranded fish (or a currency drop) could still freely swim/drift in open air, which is wrong: out of the water it should behave like any falling object, no self-propulsion, until it's back in the water. `Fish._physics_process` (`scripts/fauna/fish.gd`) now checks a new `_in_sky()` helper (true if currently above `water_surface_y` or inside an air pocket's sky zone) right after the `grabbed` early-return; while true it accumulates `_fall_velocity` via the existing `GameFeel.sub.surface_gravity` constant (the same one the sub and torpedoes already use for sky/surface gravity) and moves straight down, **skipping the entire AI state machine that frame** — no swim-toward, no horizontal drift. The instant it's back at/below the surface, `_fall_velocity` resets to zero and its previous AI state resumes normally. Same pattern added to `scripts/salvage/salvage_item.gd` (new `sky_zones`/`water_surface_y`/`_fall_velocity` fields + `_in_sky()` helper, gravity branch inside the `State.WATER` case) so a currency/scrap drop that ends up airborne falls the same way; `Fish._spawn_drop` copies its own `sky_zones`/`water_surface_y` onto each drop it spawns. New tests: `_test_falls_under_gravity_when_stranded` (`tests/test_fish.gd`, also asserts zero horizontal drift and accelerating fall speed) and `_test_falls_under_gravity_when_airborne` (`tests/test_salvage.gd`). The earlier `_is_blocked_by_sky` escape-guard (below) is now effectively superseded for the "stuck" case — the gravity short-circuit intercepts before the AI/`_swim_toward` path ever runs while airborne — but was left in place as harmless defense-in-depth. Headless-verified: no new failures.

**Falling fish/drops could clip through rock — fixed (2026-06-24):** follow-up to the gravity-fall fix above. The fall branch moved position directly with no terrain collision check, so a fish or drop falling through a cave air pocket could clip straight through the pocket's rock walls/floor. Fixed by running the fall step through the existing `_terrain_cast` ShapeCast2D first (Fish already had one for normal swimming; `SalvageItem` gained a matching one, masked to `Layers.TERRAIN`) — if the step would hit terrain, the fall stops there (`_fall_velocity` resets to 0) instead of moving into it. No new tests (no existing fixture places a fish/drop where this geometry matters); full fish/salvage regression confirmed no new failures.

**Fish in open water occasionally dropped to the rocks, as if airborne — fixed (2026-06-24):** Snir caught this right after the gravity-fall work above. Root cause: the air-pocket branch of `Fish._in_sky()`/`_is_blocked_by_sky()` (and the mirrored `SalvageItem._in_sky()`) only bounded a position by the pocket zone's **x-range** and its local `surface_y` — never by the pocket's actual rect on the y-axis. So any fish sharing an x-range with some air pocket, however far above that pocket's real cavity (fully submerged, in a totally unrelated tunnel), got falsely flagged as "in that pocket's sky" and dropped straight down under the new gravity behavior. Fixed by requiring `rect.has_point(pos)` (real 2D containment) instead of the x-only bound — this is exactly how `Sub._local_surface_y()` and `Torpedo._apply_sky_gravity()` already do it correctly elsewhere; the fish/drop code just hadn't matched that pattern. New test `_test_pocket_sky_check_bounded_by_rect` in `tests/test_fish.gd` reproduces the false-positive directly (a fish in a pocket's x-range but outside its rect must stay "in water"; one actually inside the rect must read as airborne). Headless-verified (test_fish/test_salvage/test_world/test_claw/test_telescope/test_grab_tug/test_map_loader): no new failures.

**A fish embedded in rock, frozen forever — fixed, then the fix itself fixed (2026-06-24 and 2026-06-25):** Snir found one stuck visually inside a wall. Root cause: `Fish`'s hit-knockback (`_knockback`, applied when a bullet/torpedo hits it near a wall) moved its position directly with **no terrain check at all**, so a hit near a wall could knock the fish straight into solid rock. Once embedded, every swim state's existing terrain-avoidance check then reported "colliding" for literally any direction forever — the fish's own shape already overlapped the wall at the cast's start — permanently freezing it.
- **First pass (2026-06-24):** added a `_stuck_in_terrain()` check that bypassed the per-step terrain block entirely whenever the fish was already overlapping rock, letting AI carry it back out. This worked for the original report, but Snir then caught the fish chasing him straight through a wall — the bypass re-triggered every frame the fish kept *bumping* into a wall (each bump re-overlaps it), so an AI relentlessly trying to swim into a wall could just walk through it over several frames.
- **Second pass (2026-06-25), the real fix:** removed `_stuck_in_terrain()` and its bypasses entirely — `_swim_toward()`/`State.RECOVER` are back to plain, unconditional terrain blocks. Instead, new `_try_depenetrate_from_terrain(delta)` runs once at the very top of `Fish._physics_process` (right after the `grabbed` early-return, before anything else that frame): if currently overlapping terrain, it pushes the fish back out along the wall's collision normal and **skips the entire rest of the function** — AI, sky-fall, everything — for that frame. Normal movement only ever resumes once the fish is genuinely clear, never while still overlapping, closing the exploit. The `_knockback` application's own terrain check (cancels the knockback instead of moving into a wall) still prevents new embeddings going forward.
- New tests in `tests/test_fish.gd`: `_test_escapes_when_embedded_in_terrain` (a fish dead-center in rock gets pushed out, doesn't stay frozen) and `_test_does_not_swim_through_a_wall_while_chasing` (a fish locked onto a sub on the far side of a wall, given 120 frames, never crosses to the far side). Headless-verified (test_fish/test_salvage/test_grab_tug/test_claw/test_telescope/test_world): no new failures.

**Fish (and hunters/chasers) could spot the player through solid rock — fixed (2026-06-25):** Snir noticed a chaser locking on from behind a wall as if the wall weren't there. Detection was a pure distance check (`dist_to_sub <= chaser_detect_m`/`hunter_detect_m`) with no concept of line-of-sight at all — a pre-existing design gap, not a regression from this session's other changes. New `Fish._has_line_of_sight_to_sub()` raycasts against `Layers.TERRAIN` between the fish and the nearest point of the sub's hull; both the HUNTER and CHASER initial lock-on triggers now also require this to be true. **Only gates the initial spot** — once locked on, losing line of sight again does NOT break the chase, since both behaviors are explicitly defined by refusing to let go, not by maintaining a clear sightline (re-checking continuously would have nerfed the chaser's whole "never gives up" identity). New test `_test_chaser_does_not_detect_through_walls` in `tests/test_fish.gd`. Headless-verified: no new failures.

**Checked: is the chaser's Elite-tier detection range actually larger? No.** Snir asked after observing what felt like a longer engage range on Elite chasers. Confirmed in the data: `chaser_detect_m`/`hunter_detect_m`/`territory_radius_m` (`GameFeel.fish`) are single values shared by every class tier — nothing in `EnemyClassStats` (Small/Big/Elite) varies detection range; only damage/hp/weight/size/speed/currency/elite-ability differ per class. The likely source of the perception: the Elite block's `move_speed` (6.0 m/s) is ~20% faster than Big (5.0) and ~70% faster than Small (3.5) — it closes distance much faster once locked on, which can read as "it spotted me from farther away" even though the actual lock-on radius is identical. No code change made (nothing to fix — confirmed equal); flagging here in case Snir wants per-class detection range as a deliberate future design choice instead.

**A fish landing on a pocket's own rock floor froze there forever, looking like it was "sitting on top of" rocks — fixed (2026-06-24):** Snir caught this right after the embedded-in-rock fix above — a different presentation of the same root issue. A fish falling under the new gravity behavior can legitimately land on solid ground while still inside an air pocket's sky zone (e.g. the pocket's own floor) — but `Fish._in_sky()` never turns false in that case (the fish is still above the pocket's local water line, just resting instead of falling), so the gravity branch's `if _in_sky(): ... return` kept short-circuiting every frame even after `_fall_velocity` had already hit zero, freezing it at the landing spot for good. Fixed by restructuring `Fish._physics_process`'s gravity branch: it only returns early while the *next* fall step is genuinely unobstructed; the instant that step would hit terrain, it falls through to the normal AI state machine instead (after zeroing `_fall_velocity`) rather than returning. AI's own sky/terrain escape guards (from the embedded-in-rock fix above) then work it back toward real water from the landing spot, and gravity resumes automatically the moment AI opens a path to fall through again. New test `_test_resumes_ai_after_landing_in_a_pocket` in `tests/test_fish.gd` (drops a fish onto a fake pocket's rock floor and confirms it keeps moving afterward instead of freezing). Headless-verified (test_fish/test_salvage/test_grab_tug/test_claw/test_telescope/test_world): no new failures, and the three other gravity/sky/terrain tests from this session still pass.

**Sky-stranded fish could freeze forever — fixed (2026-06-24):** earlier in the session, before the gravity fix above. Root cause: `Fish._is_blocked_by_sky(pos)` blocked any movement candidate above the water-surface line with no check on whether the fish was already up there — so once stranded, every step trying to swim back down was also rejected, since it was still nominally "in the sky" until the exact frame it crossed the line. Patched by gating the block on the fish currently being at/below the surface. Superseded in practice by the gravity fix above (which now handles all airborne movement), but the guard is still correct and left in place.

**Reference fish recolor, all the way through (2026-06-24):** Snir's call — the territorial/hunter fish (drawn purple since M0) is now **orange** end to end: body, currency drop, AND the map-authoring spawn marker (previously decoupled on purpose; Snir chose to unify them).
- `PlaceholderArt.FISH_COLOR` (`scripts/placeholder_art.gd`): `7a4ae8` (purple) -> `e8742c` (orange). `CURRENCY_COLORS` gained an `"orange"` entry (same hex) — `"teal"` is now the chaser's color (see above), not unused.
- `reference_fish.tres`: `body_color` and `currency_color` updated to match.
- `GenerationLayerParser.TERRITORIAL_FISH` (`scripts/maps/generation_layer_parser.gd`): marker hex went `#800080` (purple) -> `#E25922` (a stand-in orange) -> **`#E8742C`** (the exact hex Snir repainted the live map to, matching `PlaceholderArt.FISH_COLOR`/`EnemyDef.body_color` precisely, not just "an orange"). `tests/gen_test_map.gd` (the test-fixture generator) and the fixture it produces (`maps/test_map/test_map_gen.png`) updated/regenerated to match — **if the marker color is ever revisited, re-run `godot --headless --script res://tests/gen_test_map.gd` after editing that script, or the checked-in test fixture goes stale.**
- **Art note for Snir:** `maps/cavern_depths_01/world_01_gen.png` has 27 pixels at the new `#E8742C` but **1 leftover opaque pixel still at the old `#E25922`** (likely missed during the repaint) — that one pixel won't spawn anything since it no longer matches either marker color. Not fixed automatically (it's your art file) — worth a touch-up pass in Krita if that spawn point still matters.
- Headless-verified (test_fish/test_salvage/test_shop/test_claw/test_telescope/test_reel_minigame/test_world/test_dock_shop_ui/test_validate/test_map_loader): no new failures, same pre-existing baseline (test_fish x1, test_claw x1, test_dock_shop_ui x4, test_sub x2 confirmed pre-existing via git stash).

**Currency pickups now shape-coded by denomination (2026-06-24):** a 1/5/10/50-value pickup now reads at a glance, not just by color — each rung up `GameFeel.currency.denominations` adds a polygon edge (1=triangle, 5=square, 10=pentagon, 50=hexagon). New `SalvageItem.shape_sides_for(value)` (static, reads the ladder from `GameFeel` rather than hardcoding it) backs both the world-dropped pickup (`SalvageItem._draw`) and the telescope/claw onboard cage icons (`SubVisual._draw_currency_icon`, mirrors the same shape at icon scale).
- **Commit:** pending (all of the above, this session).
