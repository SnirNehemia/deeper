...
func handle_input(input: PlayerInput) -> void:
	...
	# with A/D and zooms with W/S. Zooming "outward" (away from the hull, into
	# open water) lengthens the beam; zooming "inward" shortens it.
	height_m = clampf(
		height_m + Station.face_zoom_input(base_dir, input) * GameFeel.floodlight.zoom_speed_m * delta,
